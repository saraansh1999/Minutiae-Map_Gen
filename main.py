...
def extract_minutiae(im_path, save_path):
	...
	with open(save_path, 'wb') as fp:
		minmap = np.zeros((im.shape[0], im.shape[1], 6))
		for t in range(num_minutiae):
			for k in range(minmap.shape[2]):
				co = minutiae[t][5] * np.exp(-dtheta(minutiae[t][3], (2*k*180/6)) / (2 * (sigmas**2)))
				for i in range(max(0, minutiae[t][2] - 3), min(minutiae[t][2] + 3, minmap.shape[0])):
					for j in range(max(0, minutiae[t][1] - 3), min(minutiae[t][1] + 3, minmap.shape[1])):
						cs = np.exp(-(np.linalg.norm(np.array([j, i]) - np.array(minutiae[t][1:3]))**2) / (2 * (sigmas**2)))
						minmap[i, j, k] += cs*co
			# print(minutiae[t][1], minutiae[t][2], minutiae[t][3], minutiae[t][5])
		minmap = minmap / np.max(minmap, axis=(0, 1))
		pickle.dump(minmap, fp)	
		# image.imsave(fp, (minmap[:, :, 3:6]))

	# with open (save_path, 'rb') as fp:
 #   		minmap = pickle.load(fp)
 #   		plt.imshow(minmap[:, :, 0])
 #   		plt.show()
		# pass
